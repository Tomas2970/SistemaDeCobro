...
if __name__ == "__main__":
    ...
    # Simular usuarios
    admin = {'id_usuario': 1, 'nombre': 'admin', 'id_rol': 1}
    supervisor = {'id_usuario': 2, 'nombre': 'supervisor', 'id_rol': 3}
    vendedor = {'id_usuario': 3, 'nombre': 'vendedor1', 'id_rol': 2}
    ...
    print("="*70)
    print(f"{'Acción':<30} {'Admin':<12} {'Supervisor':<12} {'Vendedor':<12}")
    print("-"*70)
    
    for accion in acciones_prueba:
        permisos_str = []
        for nombre, usuario in usuarios:
            tiene = "✅" if tiene_permiso(usuario, accion) else "❌"
            permisos_str.append(tiene)
        
        print(f"{accion:<30} {permisos_str[0]:<12} {permisos_str[1]:<12} {permisos_str[2]:<12}")
    
    print("="*70)
    print()
    
    # Describir cada rol
    for nombre_rol in ['admin', 'supervisor', 'vendedor']:
        print(f"\n{nombre_rol.upper()}:")
        print(describir_rol(nombre_rol))