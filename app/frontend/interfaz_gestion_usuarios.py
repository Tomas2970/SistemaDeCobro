...
from app.frontend.interfaz_crear_usuario import ui_crear_editar_usuario # Importamos el popup
...
def ui_gestion_usuarios(parent: tk.Misc, backend, usuario: dict):
        ...
        try:
            ...
            for u in usuarios:
                ...
                tree.insert("", tk.END, values=[
                    u.get('id_usuario', ''),
                    u.get('nombre', ''),
                    u.get('rol_nombre', ''), # 'rol_nombre' viene del JOIN
                    estado
                ])
    ...
    def abrir_crear_usuario():
        # Llama al popup en modo CREAR (sin usuario_existente)
        ui_crear_editar_usuario(win, backend)
        cargar_datos() # Recargar la lista al cerrar
    ...
    def on_doble_clic():
        ...
        # Necesitamos todos los datos del usuario para pasarlos al popup
        try:
            usuarios = backend.obtener_usuarios_con_rol()
            usuario_a_editar = next((u for u in usuarios if u.get('id_usuario') == id_usuario), None)
            
            if usuario_a_editar:
                # Llama al popup en modo EDITAR
    ...
    tk.Button(frame_botones, text="+ Crear Usuario", command=abrir_crear_usuario, bg="#4CAF50", fg="white", width=15).pack(side=tk.LEFT, padx=10)
    tk.Button(frame_botones, text="Desactivar Seleccionado", command=desactivar_usuario, bg="#f44336", fg="white", width=20).pack(side=tk.LEFT, padx=10)
