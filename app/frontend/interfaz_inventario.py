...
def ui_inventario(parent: tk.Misc, backend, usuario: dict) -> None:
    ...
    var_nombre = tk.StringVar(); tk.Entry(frm, textvariable=var_nombre, width=28).grid(row=0, column=1, sticky="w")
    ...
    tree.column("Código", width=140, anchor="center")
    ...
    def _norm(prod: dict) -> dict:
        return {
            ...
            "Código": prod.get("codigo_barras") or prod.get("codigo") or "",
    ...
    def _paint(filas: list[dict]) -> None:
        ...
        for p in filas:
            n = _norm(p); tree.insert("", tk.END, values=[n[c] for c in cols])
