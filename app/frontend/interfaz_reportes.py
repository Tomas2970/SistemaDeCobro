...
def _construir_panel_caja(win, backend, usuario):
    ...
    def iniciar_retiro_autorizado():
        def on_autorizado(admin_dict):
            top = ctk.CTkToplevel(win)
            top.title("Retiro Autorizado")
            top.geometry("450x420")
            top.configure(fg_color=win.col_bg)
            
            ctk.CTkLabel(top, text="Monto a retirar ($):", font=("Segoe UI", 13, "bold"), text_color=win.col_text).pack(pady=(20,5))
            ...
            def save():
                try:
                    ...
                    if m > data_c.get('efectivo_esperado', 0.0):
                        messagebox.showerror("Saldo Insuficiente", "No hay suficiente efectivo en la caja para realizar este retiro.", parent=top)
                        return
                    ...
                    if not tesoreria:
                        messagebox.showerror("Error", "No se puede transferir el efectivo porque la Tesorería está CERRADA.\nSolicite al administrador que abra la Tesorería.", parent=top)
                        return
            ...
            ctk.CTkButton(btn_frm, text="💾 GUARDAR", command=save, fg_color="#ef4444", hover_color="#dc2626", font=("Segoe UI", 15, "bold"), width=160, height=45).pack(side="right", padx=10)

            configurar_navegacion_ventana(top)
            top.grab_set()

        solicitar_autorizacion_supervisor(win, backend, usuario, on_autorizado)

    ctk.CTkButton(fr_ops, text="💸 RETIRO DE DINERO", command=iniciar_retiro_autorizado, fg_color="#6366f1", hover_color="#4f46e5", font=("Segoe UI", 13, "bold"), height=40).pack(side="left", padx=10)
